...
export function getGuestbookErrorMessage(error: unknown): string {
	...
	if (error instanceof Error) {
		...
		if (/Waline 服务地址未配置|PUBLIC_WALINE_SERVER_URL/iu.test(message)) {
			return "留言服务尚未配置，请设置 PUBLIC_WALINE_SERVER_URL 后重新构建";
		}
		if (/app is archived|应用已归档|failed with 500/iu.test(message)) {
			return "留言服务已停用，请更换为可用的 Waline 服务地址";
		}
